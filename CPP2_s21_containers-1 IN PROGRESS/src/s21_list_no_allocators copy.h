    ...
    public:
        ...
        list(size_type n) : list() {
            for(size_type i = 0; i < n; i++){
                push_back(0); // будет работать только для инт
            }
        }
        ...
        list(const list &l){
            for ( auto it : l)push_back(*it);} // предусмотреть закольцовывание в пуш

        list(list &&l){
            node_ = l.node_;
            size_ = l.size_;
            l.node_->~Node();    ///проверить??????????????
        }

        ~list(){
            clear();
            node_->~Node();   ///проверить??????????????
        }
...
// --------------------------LIST-ELEMENT-ACCESS--------------------------------------------------------------------------------//
    public:
        ...
        const_reference back(){return node_->prev_->value_;}  // проверки на наличие элемента
...
// --------------------------LIST-CAPACITY-------------------------------------------------------------------------------------//
    public:
        ...
        size_type max_size() {return std::numeric_limits<std::size_t>::max()/ sizeof(Node);}
    ...
    public:
        ...
        void unique(){
            for(iterator i = begin(); i != end(); ++i){
                while(i.pos_->next_->value_== i.pos_->value_){
                    iterator j(i);
                    erase(++j);
                }
            }
...
// --------------------------LIST--CLASSES--------------------------------------------------------------------------------------//      
    class Node{
        ...
        public:
            ...
            Node(const Node& other): value_(other.value_), next_(other.next_), prev_(other.prev_) {}
            Node(Node&& other):value_(other.value_), next_(other.next_), prev_(other.prev_) {
                other.next_ = nullptr;
                other.prev_ = nullptr;
            }
            ~Node(){
            ...
            bool operator==(const Node &other){return (next_ == other.next_ && prev_ == other.prev_ && value_ == other.value_);}// сравнение, конструкторы
            bool operator!=(const Node &other){return !(next_ == other.next_ && prev_ == other.prev_ && value_ == other.value_);}// сравнение, конструкторы
    };
